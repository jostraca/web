...
list it reads is the generator's own `reject.txt`, read straight out of the
`../jostraca` checkout (or `$JOSTRACA_REPO`) — NOT copied here, because a
second statement of one rule is a thing that drifts. CI clones the
generator for it; without that checkout the test skips with a named reason,
and the workflow asserts it did not skip. The rest of the guide — the voice,
the placement rules — is still a review obligation, and that half is what
let two banned words reach the live site before the test existed. Read it
before writing or editing an authored page.
